...
{rows && rows.length > 0}
		...
		{hasPagination && <div className="flex items-center justify-between border-t-2 border-gray-700 px-2">
				<div className="flex items-center">
					<div className="border-r-2 border-gray-700 pr-1.5">
						<div className="flex items-center pt-1.5">
							...
							>
								...
								{isItemsPerPageActive &&	<ul
										className="shadow-stack absolute inset-x-0 top-12 border-2 border-t-4 border-gray-700 bg-gray-900"
									>
										{[5, 10, 25].map((option, i) => (<li key={i}>
												<button
													className="w-full transform p-1.5 text-left transition-colors duration-150 hover:bg-gray-800"
													onClick={() => {
														page = 1;
														itemsPerPage = option;
					...
					<div className="hidden px-3 pt-1.5 text-gray-300 md:flex md:items-center">
						<p>{itemsFrom}-{itemsTo} of {handlePluralization('item', totalItems, true)}</p>
					</div>
				</div>
				<div className="flex items-center">
					<div className="flex items-center pt-1.5 pl-1.5 md:border-l-2 md:border-gray-700">
						<div
							className="relative"
							// use:handleClickOutside={{
						...
						>
							...
							/>
							{isPagesActive && <ul
									className="shadow-stack minimal-scrollbar absolute inset-x-0 top-12 max-h-28 overflow-y-auto border-2 border-t-4 border-gray-700 bg-gray-900"
								>
						...
						<p className="hidden pl-1.5 pr-3 text-gray-300 md:block">
					...
					<div className="flex items-center border-l-2 border-gray-700 pl-1.5 pt-1.5">
						<Button
							icon={<ArrowLeft/>}
							isDisabled={page < 2}
							kind="ghost"
